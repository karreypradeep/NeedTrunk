...
public class KlassServiceImpl implements KlassService {
	...
	public void removeKlass(final Klass klass) throws BusinessException, SystemException {
		...
		final Collection<Section> sections = this.sectionService.findAllSectionsByKlassId(klass.getId());
		...
		final Collection<KlassLevelFee> klassLevelFees = this.klassLevelFeeDao.findKlassFeesByKlassId(klass.getId());
		...
		final Collection<Subject> subjects = this.subjectDao.findAllSubjectsByKlassId(klass.getId());
	...
	private void validateKlass(final Klass klass) {
		final Klass pKlass = this.klassDao.findById(klass.getId());

		final Collection<KlassLevelFee> klassLevelFees = this.klassLevelFeeDao.findKlassFeesByKlassId(pKlass.getId());
		...
		for (final KlassLevelFee klassLevelFee : klassLevelFees) {
			...
			final Collection<KlassLevelFeeCatalog> klassLevelFeeCatalogs = this.klassLevelFeeCatalogDao.findKlassFeePaymentsByKlassFeeId(klassLevelFee.getId());
		...
		// this.sectionService.findActiveSectionsByKlassId(pKlass.getId());

		// if (sections == null || sections.isEmpty()) {
		// throw new BusinessException(BusinessMessages.getResourceBundleName(),
		// BusinessMessages.MSG_KLASS_CANNOT_NOT_BE_ACTIVATED_SECTIONS_ARE_NOT_DEFINED,
		// new Object[] { klass.getName() });
		// }
	...
	public void validateKlasssForAcademicYear(final Long branchId, final AcademicYear academicYear) throws BusinessException {
		...
		final Collection<Klass> klasses = this.klassDao.findActiveKlassByBranchId(branchId);
		...
		for (final Klass klass : klasses) {
			...
			final Collection<Section> sections = this.sectionService.findActiveSectionsByKlassIdAndAcademicYearId(klass.getId(), academicYear.getId());
			if ((sections != null) && !sections.isEmpty()) {
				sectionDefinedForAcademicYear = true;
				// Validate Class Fee
				final Collection<KlassLevelFee> klassLevelFees = this.klassLevelFeeDao.findKlassFeesByKlassIdAndAcademicYearId(klass.getId(),
						academicYear.getId());
				...
				for (final KlassLevelFee klassLevelFee : klassLevelFees) {
					...
					final Collection<KlassLevelFeeCatalog> klassLevelFeeCatalogs = this.klassLevelFeeCatalogDao.findKlassFeePaymentsByKlassFeeId(klassLevelFee
							.getId());
