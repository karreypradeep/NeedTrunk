/**
 * This document is a part of the source code and related artifacts for
 * SMSystem.
 * www.apeironsol.com
 * Copyright © 2012 apeironsol
 * 
 */
package com.apeironsol.need.analysis.feeDue.service;

import com.apeironsol.need.analysis.feeDue.dataobject.FeeDueAcademicYearDO;
import com.apeironsol.need.util.searchcriteria.FeeDueAnalysisSearchCriteria;

/**
 * Service interface for HostelRoom.
 * 
 * @author Sunny
 * 
 */
public interface FeeDueAnalysisService {

	FeeDueAcademicYearDO getFeeDueGeneratedForAcademicYearBySearchCriteria(final FeeDueAnalysisSearchCriteria feeDueAnalysisSearchCriteria);

}
