...
public class SMSHorizonProvider {
	...
	public String sendSMS(final String[] phoneNumbers, final String message) throws ApplicationException {
		 ...
		 * URLEncoder.encode(mobilenumber.toString(), "UTF-8");
		 * data += "&" + URLEncoder.encode("message", "UTF-8") + "=" +
		 * URLEncoder.encode(message, "UTF-8");
		 * data += "&" + URLEncoder.encode("sender", "UTF-8") + "=" +
		 * URLEncoder.encode(sid, "UTF-8");
		 * data += "&" + URLEncoder.encode("reqid", "UTF-8") + "=" +
		 * URLEncoder.encode(reqid, "UTF-8");
		 * data += "&" + URLEncoder.encode("route_id", "UTF-8") + "=" +
		 * URLEncoder.encode(route_id, "UTF-8");
		 * 
		 * // Push the HTTP Request
		 * URL url = new
		 * URL("http://clients.smshorizon.in/API/WebSMS/Http/v1.0a/index.php");
