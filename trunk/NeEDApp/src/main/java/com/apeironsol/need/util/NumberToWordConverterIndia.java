 ...
 * 
 */
public class NumberToWordConverterIndia implements NumberToWordConverter {
	...
	public String convertNumberToWord(final long number) {
		...
		if (number >= 0 && number < 21) {
			...
			reminder = (int) number % 100000;
			leftOverNumber = (int) ((number - reminder) / 100000);
			return this.convertNumberToWord(leftOverNumber) + this.appendAndToWordIfRequired(reminder, leftOverNumber == 1 ? " Lakh " : " Lakhs ")
					+ (reminder > 0 ? this.convertNumberToWord(reminder) : "");
		} else if (number > 9999999 && number < 1000000000) {
			reminder = (int) number % 10000000;
			leftOverNumber = (int) ((number - reminder) / 10000000);
			return this.convertNumberToWord(leftOverNumber) + this.appendAndToWordIfRequired(reminder, leftOverNumber == 1 ? " Crore " : " Crores ")
					+ (reminder > 0 ? this.convertNumberToWord(reminder) : "");
		}
	 ...
	 * context is any of: lakh, thousand or hundred
	 * 
	 * @param reminder
	 *            reminder.
	 * @param word
	 ...
	 */
	private String appendAndToWordIfRequired(final int number, final String word) {
		return number < 100 && number > 0 ? word + "And " : word;
	}
