...
@Named
@Scope(value = "session")
public class BranchAdmissionsNotificationsBean extends AbstractNotificationBean {
	 ...
	 * Default constructor.
	 */
	public BranchAdmissionsNotificationsBean() {
	}
		...
		if (this.getNotificationTypeConstant() == null) {
			...
			try {
				...
				this.scheduledBatchLog = new BatchLogBuilder().branch(this.sessionBean.getCurrentBranch())
						...
						.notificationSentFor(NotificationSentForConstant.ADMISSIONS).messageToBeSent(this.getNotificationText())
						.notificationSendForAcademicYear(this.getAcademicYearForNotification()).exam(this.getSelectedExamForNotification())
						.attendanceDate(DateUtil.getSystemDate()).smsProvider(this.sessionBean.getCurrentBranchRule().getSmsProvider()).build();
				...
				this.scheduledBatchLog = this.notificationService.sendNotificationForStudentAdmissions(this.scheduledBatchLog);
	...
	public void loadBatchLogsByBranchLevelAndBranchId() {
		if (this.loadBatchLogsFromDB) {
			this.setKlassBatchLogs(this.batchLogService.findBatchLogsByNotificationLevelAndNotificationLevelId(this.sessionBean.getCurrentBranch().getId(),
					NotificationLevelConstant.BRANCH, this.sessionBean.getCurrentBranch().getId(), NotificationSubTypeConstant.getAdmissionNotifications(),
					NotificationSentForConstant.ADMISSIONS));
	...
	public void loadExamsForNotifications() {
		ViewUtil.addMessage("Notification  not supported for admission.", FacesMessage.SEVERITY_WARN);
	}
	...
	public void getBranchNotificationByNotificationType() {
		...
		for (final BranchNotification branchNotification : this.getBranchNotifications()) {
			if (NotificationSubTypeConstant.getAdmissionNotifications().contains(branchNotification.getNotificationSubType())) {
