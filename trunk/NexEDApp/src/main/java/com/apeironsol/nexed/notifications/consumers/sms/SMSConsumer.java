 ...
 */
package com.apeironsol.nexed.notifications.consumers.sms;
...
import com.apeironsol.nexed.notifications.consumers.worker.sms.SMSWorker;
import com.apeironsol.nexed.notifications.consumers.worker.sms.SMSWorkerFactory;
...
public class SMSConsumer implements SessionAwareMessageListener<Message> {
	...
	private void resendLastMessage(final BatchLog batchLog) {
		NexEDJMSObject newJmsObject = new NexEDJMSObject(batchLog.getId());
	 ...
	 */
	private void processBatchMesssage(final NexEDJMSObject jmsObject, final BatchLog batchLog) throws Exception {
		try {
