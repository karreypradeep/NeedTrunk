...
import com.apeironsol.nexed.core.service.BranchExportService;
import com.apeironsol.nexed.util.portal.ViewExceptionHandler;
import com.apeironsol.framework.exception.ApplicationException;

@Named
@Scope(value = "session")
public class ExportStudentsBean extends AbstractPortalBean {

	/**
	 * Unique serial version id for this class.
	 */
	private static final long			serialVersionUID	= -3855029673124809055L;
	...
	public StreamedContent getExcelAfterImport() {
		...
		try {
			final byte[] workbook = this.branchExportService.exportStudents(this.selectedAcademicYear, this.selectedClass, this.selectedSection);
			final InputStream stream = new ByteArrayInputStream(workbook);
			streamedContent = new DefaultStreamedContent(stream, "application/xls", "ExportStudents.xls");
		} catch (final ApplicationException e) {
			ViewExceptionHandler.handle(e);
		}
